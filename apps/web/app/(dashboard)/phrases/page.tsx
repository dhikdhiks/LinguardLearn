      ...
      {filteredPhrases.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredPhrases.map((p) => (
            <div key={p.id} className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition group">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">{p.phrase}</h2>
                    <button
                  ...
                  {p.tags && p.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {p.tags.map(tag => <span key={tag} className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full">#{tag}</span>)}
                    </div>
                  )}
                </div>
              ...
              <div className="mt-3">
                ...
                {p.notes && <p className="text-xs text-gray-400 mt-1 border-t border-gray-100 dark:border-gray-700 pt-1">📝 {p.notes}</p>}
              </div>
            </div>
          ))}
        </div>
